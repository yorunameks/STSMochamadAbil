...
function tampildata()
    ...
    {
        global $koneksi;
        $hasil=mysqli_query($koneksi,"SELECT barang.id_brg, barang.kode_brg, barang.nama_brg, barang.kategori, barang.merk, barang.jumlah from barang INNER JOIN user on barang.id_brg=user.id_user;");
        $rows=[];
        while($row = mysqli_fetch_assoc($hasil))
        {
            $rows[] = $row;
    ...
    {
        global $koneksi;
        $hasil=mysqli_query($koneksi,"SELECT * FROM barang WHERE id_brg='$id_user';");
        $rows=[];
        while($row = mysqli_fetch_assoc($hasil))
        {
            $rows[] = $row;
